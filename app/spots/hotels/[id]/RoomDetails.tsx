...
export default function RoomDetails() {
  ...
  useEffect(() => {
    const room = roomsDummyData.find((room) => room._id === id) as
      | RoomType
      | undefined;
    if (room) {
      setRoom(room);
  ...
  const handleCheckAvailability = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isRoomAvailable()) {
      ...
      router.push(
        `/spots/bookings?roomId=${room._id}&checkIn=${checkIn}&checkOut=${checkOut}&guests=${guests}`
      );
    } else {
...
  const selectedHotel = room.hotel;
...
    (1000 * 60 * 60 * 24);
  const calculatedPrice = days * selectedRoom.pricePerNight;
  ...
  return (
    <div className="py-28 md:py-36 px-4 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
        ...
        <p className="text-sm py-1.5 px-3 bg-orange-500 text-white rounded-full">
          20% off
        </p>
      </div>
          ...
          {room.images.length > 1 &&
            room.images.map((image, index) => (
              <Image
                ...
                height={300}
                alt={room.hotel.name}
                className={`w-full rounded-xl overflow-hidden cursor-pointer shadow-2xl hover:shadow-3xl transition-all duration-300 ease-in-out ${
      ...
      <form
        ...
        className="flex flex-col md:flex-row items-start md:items-center justify-between shadow-xl p-6 rounded-xl mx-auto mt-16 max-w-6xl"
      >
    ...
    Book Now
      ...
      <div className="max-w-3xl border-y-2 border-yellow-300 my-15 py-10">
        <p>
          ...
          Availability. Your get a comfortable two bedroom apartmennt has a true
          city feeling. The price quoted is for two guest, at the guest slot
          please mark the number of guests to get the exact price for groups .
          The Guests will be allocated ground floor accordinng to availibility.
          You get the comfortable two bedroom apartment has a true city feeling.
        </p>
      </div>
